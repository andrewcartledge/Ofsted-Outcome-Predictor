...
```{r Nearly final filter before modelling}
...
Test models, first random forest
...
```{r Testing the models - Logistic Regression - 1 line}
...
```{r Completing the Logistic Regression modelling}

model_lr_outcome <- predict(log_reg_model, warwickshire_future, type = "prob")

```

```{r Tidy up a bit part 2}
...
```{r Combine predictions to original data and write out}
